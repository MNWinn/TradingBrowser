...
Run from the TradingBrowser directory:
    cd /home/mnwinnwork/.openclaw/workspace/TradingBrowser
    python3 backend/setup_paper_trading.py
"""
...
try:
    ...
    print("Make sure you're running this from the TradingBrowser/backend directory")
    sys.exit(1)
...
async def test_alpaca_connection():
    ...
    try:
        ...
        if "error" in account:
            ...
            if account.get('http_status') == 401:
                print("\n  Your API credentials appear to be invalid or expired.")
                print("  Please generate new keys at https://alpaca.markets/")
            return False
...
async def test_practice_trading():
    ...
    print("\n" + "="*60)
    print("STEP 4: Testing Practice Trading Engine")
    print("="*60)
    ...
    try:
        ...
        # Create portfolio
        print("\n  Creating practice portfolio...")
        portfolio = engine.create_portfolio(
            user_id=user_id,
            initial_balance=Decimal("100000.00")
        )
        print(f"  ✓ Portfolio created: ${portfolio.initial_balance}")
        
        # Execute a test trade
        print("\n  Executing test paper trade (AAPL x 1 @ $175.00)...")
        ...
        print(f"  ✗ Practice trading test failed: {e}")
        return False
...
8. VIEW AVAILABLE STRATEGIES:
   curl http://localhost:8000/practice/strategies \\
     -H "Authorization: Bearer analyst-dev-token"
""")
...
async def main():
    ...
    # Step 4: Test practice trading (always works)
    practice_ok = await test_practice_trading()
    ...
    print(f"  Practice Engine: {'✓ OK' if practice_ok else '✗ FAILED'}")
    
    if practice_ok:
        print("\n✓ Practice trading is ready to use!")
    if creds_ok and connection_ok:
        print("✓ Alpaca paper trading is ready to use!")
    elif creds_ok and not connection_ok:
        print("\n⚠ Alpaca credentials present but connection failed.")
        print("  You can still use the practice trading engine.")
